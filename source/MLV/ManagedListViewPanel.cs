...
namespace MLV
{
    ...
    public partial class ManagedListViewPanel : Control
    {
        ...
        public bool AllowColumnClick = true;
        /// <summary>
        ...
        public bool AllowItemMultiSelect;
        /*Colors and images*/
        ...
        public bool IsItemVisible(int itemIndex)
        {
            ...
            return itemIndex >= lineIndex && itemIndex <= (lines + lineIndex - 4);
        }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                ...
                {
                    AfterColumnResize?.Invoke(this, new EventArgs());
                }
                if (moveType == ManagedListViewMoveType.Column && selectedColumnIndex >= 0 && isMovingColumn)
                {
                    ...
                    AfterColumnReorder?.Invoke(this, null);
                }
            }
            #endregion
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (viewMode == ManagedListViewViewMode.Details)
            {
                ...
                if (e.Y <= columnHeight)
                {
                    SwitchToColumnsContextMenu?.Invoke(this, new EventArgs());
                    ...
                    if (e.Button == System.Windows.Forms.MouseButtons.Left)
                    {
                        if (moveType == ManagedListViewMoveType.ColumnVLine && selectedColumnIndex >= 0)
                        {
                            ...
                            if (previousShift != shift & originalcolumnWidth + shift > 5)
                            {
                                ...
                                columns[selectedColumnIndex].Width = originalcolumnWidth + shift;
                                buffered_columnsSize = CalculateColumnsSize();// reset buffer
                                RefreshValues?.Invoke(this, null);
                                previousShift = shift;
                ...
                {
                    SwitchToNormalContextMenu?.Invoke(this, new EventArgs());
                    if (e.Button == System.Windows.Forms.MouseButtons.Left)
                    {
                        if (moveType == ManagedListViewMoveType.ColumnVLine && selectedColumnIndex >= 0)
                        {
                ...
                if (e.Y > columnHeight)
                {
                    if (e.Button == System.Windows.Forms.MouseButtons.Left)
                    {
                        ...
                        if (DrawSelectRectangle && AllowItemMultiSelect)
                        {
                            ...
                            if (ModifierKeys != Keys.Control)
                            {
                                ...
                                if (raiseEvent && !clear_event_raisen)
                                {
                                    clear_event_raisen = true;
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            {
                #region item select
                if (e.Y > columnHeight && moveType == ManagedListViewMoveType.None)
                {
                    if ((e.X < buffered_columnsSize - HscrollOffset))
                    {
                        if (e.Button == System.Windows.Forms.MouseButtons.Left && !isMovingColumn)
                        {
                            if (!isSecectingItems)
                            {
                                if (overItemSelectedIndex >= 0)
                                {
                                    ...
                                    bool moreThanOneSelected = SelectedItems.Count > 0;
                                    bool isShiftSelection = false;
                                    if (AllowItemMultiSelect)
                                    {
                ...
                if (e.Y < columnHeight)
                {
                    if (e.Button == System.Windows.Forms.MouseButtons.Left)
                    {
                        if (moveType == ManagedListViewMoveType.Column && selectedColumnIndex >= 0 && !isMovingColumn && AllowColumnClick)
                        {
            ...
            {
                if (e.Button == System.Windows.Forms.MouseButtons.Left && !isMovingColumn && items.Count > 0 && !isSecectingItems)
                {
                    if (overItemSelectedIndex >= 0)
                    {
                        bool currentItemStatus = items[overItemSelectedIndex].Selected;
        ...
        private void DrawDetailsView(PaintEventArgs pe)
        {
            ...
            foreach (ManagedListViewColumn column in columns)
            {
                ...
                if (cX >= HscrollOffset)
                {
                    ...
                    if (highlightSelectedColumn && selectedColumnIndex == i && AllowColumnClick)
                    {
                        if (!isMouseDown)
                    ...
                    pe.Graphics.FillRectangle(new SolidBrush(Hcolor),
                            new Rectangle(x - HscrollOffset + 1, 1, column.Width, columnHeight));
            ...
            if (DrawGridLines)
            {
                for (int j = 0; j < lines; j++)
                {
                    int yg = (j * itemHeight) - offset + columnHeight;
                    if (yg > columnHeight)
                        pe.Graphics.DrawLine(new Pen(Brushes.LightGray), new Point(0, yg), new Point(this.Width, yg));
                }
            }
            if (columns.Count > 0)
