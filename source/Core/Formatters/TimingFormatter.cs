...
{
    /// <summary>
    /// Class for timing formatting and converting.
    /// </summary>
    public sealed class TimingFormatter
    {
        /* ! FORMATTING !
         * 
         * h-m-s-i-fxx-sfxx-n
        ...
        public static string ParseCode(string c, double v)
        {
            // Get value times
            if (c.StartsWith("sff"))
            {
                ...
                if (!double.TryParse(c.Substring(3, c.Length - 3).Replace('_', '.'), out frameRate))
                    throw new Exception("Error in given timing format: not valid framerate." + " '" + c + "'");
            ...
            switch (c)
            {
                ...
                default: throw new Exception("Unable to parse timing format: invalid parameter.");
            }
        }
        public static double ParseCodeNumeric(string c, double v)
        {
            // Get value times
            if (c.StartsWith("sff"))
            {
        ...
        public static string SecondsToTime(double value, string format)
        {
            ...
            // Check
            if (format.Length == 0)
                throw new Exception("Error: timing format cannot be empty !!");
        ...
        public static double TimeToSeconds(string value, string format)
        {
            ...
            if (formatSplit.Length != valueSplit.Length)
                throw new Exception("Given value doesn't match given timing format.");
            for (int i = 0; i < formatSplit.Length; i++)
            {
                int v = 0;
                if (!int.TryParse(valueSplit[i], out v))
                    throw new Exception("Error in given value: unable to cast integer from" + " '" + valueSplit[i] + "'");
                if (formatSplit[i].ToLower().StartsWith("sff"))
                {
                    ...
                    if (!double.TryParse(formatSplit[i].Substring(3, formatSplit[i].Length - 3).Replace('_', '.'), out frameRate))
                        throw new Exception("Error in given timing format: not valid framerate." + " '" + formatSplit[i] + "'");
